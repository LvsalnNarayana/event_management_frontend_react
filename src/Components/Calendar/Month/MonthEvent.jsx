...
const MonthEvent = ({ event }) => {
  ...
  const formatEventTime = (event, transform, height) => {
    ...
    // Validate transform.y and height
    const transformY = typeof transform?.y === "number" ? transform.y : 0;
    const eventHeight = typeof height === "number" && height > 0 ? height : 0;

    // Calculate minutes to add (ensure non-negative)
    const startMinutes = Math.max(0, (transformY * 60) / 48 || 0);
    // Default to 15 minutes if height is 0
    const endMinutes = Math.max(0, (eventHeight * 60) / 48 || 15);

    // Calculate start and end times
    const formattedStartTime = format(
      addMinutes(new Date(startTimeStr), startMinutes),
      "hh:mm aa",
    );

    const formattedEndTime = format(
      addMinutes(new Date(startTimeStr), endMinutes),
      "hh:mm aa",
    );
      ...
      >
        {!isDragging && (
          ...
          >
            ...
            >
              {`${event?.title} - (${formatEventTime(event, transform, height)})`}
            </Typography>
          </ResizableBox>
        )}
        {isDragging && (
